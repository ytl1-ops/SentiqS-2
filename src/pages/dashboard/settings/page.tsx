...
import RejectionLogPanel from './components/RejectionLogPanel';
import OrgBrandingPanel from './components/OrgBrandingPanel';
import PurgeSyntheticPanel from './components/PurgeSyntheticPanel';

type TabId = 'overview' | 'alerts' | 'subscribers' | 'subscriptions' | 'traffic' | 'payments' | 'links' | 'audit' | 'branding' | 'maintenance';
...
export default function SettingsPage() {
  ...
  const tabs: { id: TabId; label: string; icon: string }[] = [
    ...
    { id: 'audit', label: isFr ? 'Journal des rejets' : 'Rejection Log', icon: 'ri-shield-keyhole-line' },
    { id: 'branding', label: isFr ? 'Marque Blanche' : 'White Label', icon: 'ri-palette-line' },
    { id: 'maintenance', label: isFr ? 'Maintenance' : 'Maintenance', icon: 'ri-tools-line' },
  ];
  ...
  const overviewCards: {
    ...
    {
      title: isFr ? 'Journal des rejets DB' : 'DB Rejection Log',
      desc: isFr ? 'Historique des insertions rejetées par le trigger de validation feeds. Chaque tentative suspecte est tracée avec son motif.' : 'History of insertions rejected by the feeds validation trigger. Every suspicious attempt is logged with its reason.',
      icon: 'ri-shield-keyhole-line',
      iconBg: 'bg-red-50',
      iconColor: 'text-red-500',
      tabId: 'audit',
      actionLabel: isFr ? 'Consulter le journal' : 'View log',
      actionIcon: 'ri-file-search-line',
    },
  ];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="anim-entry-scale" style={{ animationDelay: '100ms' }}>
        ...
        {activeTab === 'audit' && <RejectionLogPanel />}
        {activeTab === 'branding' && <OrgBrandingPanel />}
        {activeTab === 'maintenance' && <PurgeSyntheticPanel />}
      </div>
    </div>
  );
}