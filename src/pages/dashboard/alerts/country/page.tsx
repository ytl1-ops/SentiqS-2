...
export default function CountryAlertDetailPage() {
  ...
  const countryName = COUNTRY_NAME_BY_CODE[countryCode?.toUpperCase() || ''] || countryCode || t('dashboard.alerts.unknown');
  ...
  useEffect(() => {
    if (!countryName || countryName === t('dashboard.alerts.unknown')) {
      setLoading(false);
      setError(t('dashboard.alerts.countryNotFound', { code: countryCode || '' }));
      return;
    }
