...
export default function SituationCorrelations() {
  ...
  const confidenceBadge = (confidence: number) => {
    if (confidence >= 90) return 'text-emerald-600 font-bold';
    if (confidence >= 75) return 'text-yellow-600 font-bold';
    if (confidence >= 50) return 'text-orange-600 font-bold';
    return 'text-gray-400';
  };

  const confidenceBg = (confidence: number) => {
    if (confidence >= 90) return 'bg-emerald-50 border-emerald-200';
    if (confidence >= 75) return 'bg-yellow-50 border-yellow-200';
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-2">
          {topCorrelations.map((corr: SupabaseCorrelation) => {
            const isLowConfidence = corr.confidence < 50;
            return (
            <div
              key={corr.id}
              className={`rounded-lg border p-3 hover:border-gray-200 transition-colors ${confidenceBg(corr.confidence)}`}
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                    ...
                    {isLowConfidence && (
                      <span className="px-2 py-0.5 rounded text-[10px] font-semibold border bg-gray-100 text-gray-500 border-gray-300">
                        <i className="ri-error-warning-line text-[9px] mr-0.5" />
                        {t('dashboard.correlations.lowConfidence', 'Faible confiance')}
                      </span>
                    )}
                  </div>
                  <p className={`text-xs font-medium leading-snug ${isLowConfidence ? 'text-gray-500' : 'text-sentiqs-navy'}`}>
                    {isLowConfidence && (
                      <i className="ri-information-line text-gray-400 mr-1 text-[10px]" />
                    )}
                    {corr.alert_title}
                  </p>
                  <p className={`text-[11px] mt-0.5 leading-relaxed line-clamp-2 ${isLowConfidence ? 'text-gray-400' : 'text-gray-500'}`}>{corr.description}</p>
                ...
                <div className="text-right flex-shrink-0">
                  <div className={`text-sm ${confidenceBadge(corr.confidence)}`}>{corr.confidence}%</div>
