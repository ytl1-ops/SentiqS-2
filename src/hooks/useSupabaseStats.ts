...
export interface DashboardStats {
  ...
  responseTime: string;
  /** Compteurs détaillés pour transparence anti-hallucination */
  verifiedAlerts: number;
  unverifiedAlerts: number;
  rumorFeeds24h: number;
}
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      const [
        ...
        supabase.from('alerts').select('verification_status').eq('status', 'active'),
        supabase.from('feeds').select('*', { count: 'exact', head: true }).gte('timestamp', new Date(Date.now() - 86400000).toISOString()),
        supabase.from('feeds').select('verification_status').gte('timestamp', new Date(Date.now() - 86400000).toISOString()),
      ...
      // Compteurs anti-hallucination
      const verifiedAlertsCount = (activeAlertsData || []).filter((a: { verification_status: string }) =>
        isVerifiedData({ verification_status: a.verification_status })
      ).length;
      const unverifiedAlertsCount = (activeAlertsData || []).filter((a: { verification_status: string }) =>
        a.verification_status === 'unverified'
      ).length;
      const rumorFeeds24hCount = (feeds24hData || []).filter((f: { verification_status: string }) =>
      ...
      const alertEvents: TimelineEvent[] = (recentAlerts || []).map((a: { id: string; title: string; severity: string; locality: string; department: string; timestamp: string; source: string; country: string }) => ({
        id: `ALT-${a.id}`,
        ...
        source: a.source,
        navigateTo: `/dashboard/alerts/country/${a.country}`,
      }));
      const feedEvents: TimelineEvent[] = (recentFeeds || []).slice(0, 8).map((f: { id: string; title: string; timestamp: string; locality: string; department: string; source: string }) => ({
        id: `FLX-${f.id}`,
      ...
      setStats({
        ...
        totalMonitored: 54,
        responseTime: '2.3min',
        verifiedAlerts: verifiedAlertsCount,
        unverifiedAlerts: unverifiedAlertsCount,
        rumorFeeds24h: rumorFeeds24hCount,
      });
    } catch (err) {
