...
export function useLinkStatus() {
  ...
  const [integrityGuardResult, setIntegrityGuardResult] = useState<{
    scanned: number;
  ...
  const runScan = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (invokeError) throw new Error(invokeError.message || 'Scan failed');
      if (!result || !result.success) {
        throw new Error((result && result.error) || 'Scan failed');
      }

      if (!silent && result.summary) {
        setScanSummary({
          total: result.summary.total || 0,
          active: result.summary.active || 0,
          warning: result.summary.warning || 0,
          dead: result.summary.dead || 0,
          checked_at: result.summary.checked_at || new Date().toISOString(),
          results: [],
        });
      }

      if (!silent && result.integrity_guard && !result.integrity_guard.error) {
        setIntegrityGuardResult({
          scanned: result.integrity_guard.scanned || 0,
          violations_found: result.integrity_guard.violations_found || 0,
          auto_fixed: result.integrity_guard.auto_fixed || 0,
